...
<div class="page-main">
	...
	<div>
		...
		<c:forEach var="entry" items="${categories}">
			<a href="list?dcate_num=${entry.key}">${entry.value}</a> |
		</c:forEach>
	</div>
				...
				<c:forEach var="goods" items="${list}">
					<c:if test="${sessionScope.user != null && sessionScope.user.mem_status == 9 || goods.item_status == 1}">
						<tr>
							...
							<td class="align-center"><img src="${pageContext.request.contextPath}${goods.item_photo}" class="my-photo" width="100px" height="100px"></td>
							<td class="align-center"><c:out value="${categories[goods.dcate_num]}"/></td>
							<td class="align-left"><a href="detail?item_num=${goods.item_num}">${goods.item_name}</a></td>
							<td class="align-center">${goods.item_price}</td>
							<td class="align-center">${goods.item_stock}</td>
							<c:if test="${sessionScope.user != null && sessionScope.user.mem_status == 9}">
								<td class="align-center"><a href="${pageContext.request.contextPath}/goods/update?item_num=${goods.item_num}">상품정보 변경</a>
