...
<br>
    ...
    <form action="list" id="search_form" method="get">
        ...
        <div class="align-right">
           ...
           <c:if test="${sessionScope.user != null && sessionScope.user.mem_status == 9}">
    			<a href="write" class="default-btn btn-large">상품 등록</a>
    			<a href="${pageContext.request.contextPath}/admin/purchaseList" class="default-btn btn-large">구매 관리</a>
		   </c:if>

		<c:if test="${sessionScope.user != null}">
    		<a href="purchaseHistory" class="default-btn btn-large">내 구매내역</a>
    		<a href="${pageContext.request.contextPath}/cart/list" class="default-btn btn-large">장바구니</a>
    	</c:if>
        </div>
    </form>
    <br>
    ...
    <div class="category-buttons">
        <a href="list" class="btn btn-outline-info btn-category">전체</a>
        <c:forEach var="entry" items="${categories}">
            <a href="list?dcate_num=${entry.key}" class="btn btn-outline-info btn-category">${entry.value}</a>
        </c:forEach>
    </div>
    <c:if test="${count == 0}">
                ...
                <c:if test="${sessionScope.user != null && sessionScope.user.mem_status == 9 || goods.item_status == 1}">
                    <div class="product-item">
                        ...
                        <div class="product-details">
                            ...
                            <span class="product-name">상품명:
								<a href="detail?item_num=${goods.item_num}">${goods.item_name}</a>
							</span> <span class="product-price">가격: <fmt:formatNumber
									value="${goods.item_price}" type="number" groupingUsed="true" /> 원
							</span> 
							<span class="product-stock">재고: <fmt:formatNumber
									value="${goods.item_stock}" type="number" /> ea
							</span>
						</div>
                        <c:if test="${sessionScope.user != null && sessionScope.user.mem_status == 9}">
